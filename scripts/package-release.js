...
fs.copyFileSync(
  path.join(root, "dist", "code.js"),
  path.join(stageDir, "dist", "code.js"),
);
fs.copyFileSync(
  path.join(root, "dist", "ui.html"),
  path.join(stageDir, "dist", "ui.html"),
);
...
console.log(
  `Packaged dist-release/export-pro.zip (contains only what's needed to install: manifest.json + dist/)`,
);
